...
class NaiveBayesClassifer():
    ...
    def train(self, x, y):
        ...
        if self.evaluate:
            ...
            clf.fit(x, y)
        # Model dumping
        joblib.dump(clf, "./results/classifier_models/nbc")
        print("training succeedd!")
        print("the model is saved to /results/classifier_models/nbc")
    ...
    def predict(self, x):
        ...
        """
        # Model loading
        clf = joblib.load("./results/classifier_models/nbc")
        # Features loading
        stat = self.fe.load_statistic_features(x, x)
        if not self.meta_loaded:
            x =  stat["test_count"]
